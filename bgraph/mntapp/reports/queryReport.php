	...
	<div class="panel panel-default">
		...
		<div class="panel-body">
			...
			<fieldset class="col-md-12">
				...
				<div class="panel panel-default">
					<div class="panel-body">
						<form class="form-horizontal " action="./dwn_report" method="post" id="reportForm">
							<div class="row col-sm-12">
								...
								<div class="col-sm-2">
									<div class="form-group">
									<label for="report_type" class="control-label">Select Report</label>
										<select	id="report_type" name="report_type" class="form-control">
											<option></option>
											<option value="maint_report">Maintenance Report</option>
											<option value="checkl_report">Checklist Report</option>											
										</select>
									</div>
								</div>
							</div>
							<div class="col-sm-2">									
								    <button type="submit" name="download_file" id="download_file" class="btn btn-success btn-sm mr-1">Download report </button>
							</div>
						</form>
					</div>
				</div>
...
$fname =$milliseconds.'_'.'MaintenanceReport'.'.pdf';
...
if('maint_report'.localeCompare(rt)==0){
	
	var dat = $('#starts_at').val();
    ...
    $.ajax({
        ...
        success: function(data) {                 
        	...
        	var dataContent = buildImageContent(jsonData);
			downloadReport(dataContent);        	
        },
    	error: function(err) {            	
    		console.log(err);          
        }
    ...
    function buildImageContent(jsonData){
    	var items = {};
    	// ...and then:
    	var item = new Array();
		
    	for (var i = 0; i < jsonData.length; i++) {
    	    var temp_item = jsonData[i];			
    	    ...
    	    // temp_item.name = 'some value'
    	  	 ...
    	  	 item.push( 

    	  			{
    			        ...
    			        alignment:'center',
    			        fontSize:10,
    			        text:'Monthly Maintenance Before and After Photos-'+months[dt.getMonth()]+' '+ dt.getFullYear()
    			      ...
    			      columns: [
    			        {
    			          ...
    			          text: 'Station ID: '+temp_item.station_id
    			        },
    			        {
    			          // star-sized columns fill the remaining space
    			          // if there's more than one star-column, available width is divided equally
    			          width: '*',
    			          text: 'Station name: '+temp_item.station_name
    			        },
    			        {
    			          // fixed width
    			          width: '*',
    			          text: 'Maintenance Date & Time: '+temp_item.start_date
    			        }
    			      ],
...

    	    	{
		          ...
		          "margin" : [ 20, 10, 10,20],
		          width: '*',
		          text: 'Before maintenance'
		        },{
        	  	 "image":temp_item.before_image,
    	        "margin" : [ 20, 10, 10,20],
    	        "alignment"  : 'left',
    	        "fontSize":10,
    	        "width": 450,
