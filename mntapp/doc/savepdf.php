...
if(!empty($_FILES['pdf'])){
    ...
    $sql = "UPDATE `tasks` SET `status`='$status',`comments`='$comment',`end_date`='$maint_date' WHERE `id`='$task_id'";
   
    
    $conResult = mysqli_query($con, $sql) or die(mysqli_error($con));
    
    // update mnt_report
    $qry = "UPDATE `mnt_report` SET `html`='$doc2Html',`pdf`='$filepath', `maintenance_date`='$maint_date' WHERE `checklist_id`='$checklist_id'";
