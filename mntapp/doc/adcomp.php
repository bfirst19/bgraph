		...
		<div id="container5">
			<div id="row3" class="row col-sm-12">
				...
				<div class="col-sm-4">
					<div class="form-group">
						<p>
							<b>Station name</b>:<?php echo $st_name;?>
						</p>
					</div>
				</div>
				<div class="col-sm-4">
					<div class="form-group">
						<p>
							<b>Maintenance Date &amp; Time</b>: <?php echo $main_time;?>
						</p>
					</div>
				</div>
			</div>
...
if (isset($_GET['id'])) {
    ...
    for ($i = 0; $i < sizeof($someArray); $i ++) {
        ...
        if (sizeof($rowdata["components"]) > 0) {
            ...
            $isSign = strcmp($rowdata["legend"], "Signature");
            $isRemarks = strcmp($rowdata["legend"], "Remarks");
            $isbMaintenance = strcmp($rowdata["legend"], 'Before Maintenance');
            $isaMaintenance = strcmp($rowdata["legend"], 'After Maintenance');
             //echo $rowdata["legend"] . 'legend:' . $isMaintenance . '-' . $isSign . '-' . $isRemarks;           
            if (($isRemarks != 0) && ($isbMaintenance != 0) && ($isaMaintenance != 0) && ($isSign == 0)){            
                //print_r($rowdata["components"]);
                for ($k = 0; $k < sizeof($rowdata["components"]); $k++) {
                    $desc = $rowdata["components"][$k]["description"]; 
                    if (strcmp($desc, "user signature") == 0) {
                        $usignature = $rowdata["components"][$k]["currentValue"];                   
                    }
                    if (strcmp($desc, "manager signature") == 0) {
                        $psignature = $rowdata["components"][$k]["currentValue"];
                    }
                }
            }
            
            if (($isRemarks == 0) && ($isbMaintenance != 0) && ($isaMaintenance != 0) && ($isSign != 0)){
                    $remarks = $rowdata["components"][0]["currentValue"];
                    //echo $remarks;
...
?>
		<div id="report_image" class="row">
			...
			<p>Before Maintenance:</p>
				<?php
    if (isset($bfImageData) && ! empty($bfImageData)) {
        echo '<img src="data:image/gif;base64,' . $bfImageData . '" alt="Before Maintenance:" height="50%" width="50%" />';
    }
    ...
    if (isset($afImageData) && ! empty($afImageData)) {
        echo '<img src="data:image/gif;base64,' . $afImageData . '" alt="After Maintenance:" height="50%" width="50%" />';
    }
		...
		<div id="container15">
			<div id="row2" class="row col-md-12">
				<div id="mainTeam" class="col-md-6" align="left">
					...
					<p contenteditable="false" id="usignature">
						Sign:<?php 
						echo '<img src="' . $usignature . '" alt="" height="40%" width="30%" />'; 
						?>
					</p>
				</div>
				...
				<div id="verifyTeam" class="col-md-6" align="left">
					...
					<p contenteditable="false" id="psignature">
						Sign:<?php echo '<img src="' . $psignature . '" alt="" height="30%" width="40%" />';
						?>
					</p>
				</div>
			</div>
...
$(document).ready( function() {
		...
		var dc =[
			   ...
			   },			   
			   {
					...
					table: {
						widths: [ '50%', '40%'],
						body: [							
							[{text:'Maintenance Team',alignment : 'left',}, {text:'Verified By', alignment : 'right',}],
							[{text:'Name: <?php echo $user1;?>', alignment : 'left',}, {text:'Name: <?php echo $user2;?>',alignment : 'right', }],
							[
							    {
						            image:'<?php echo $usignature;?>',						            
						            fit:[200,100],
						            alignmnt : 'left',
						          
						         },
						         {
						        	image:'<?php echo $psignature;?>',						            
						            fit:[200,100],
						            alignment : 'right',
	...
	$('#editReport').on('click', function(e){
    	...
    	$('#checklist_table_id').css({"border-style": "outset"});
    	$('#remarks_report').prop('contenteditable', true);
    	$('#remarks_report').css({"border-style": "outset"});

    	/*$('#usignature').prop('contenteditable', true);
    	$('#usignature').css({"border-style": "outset"});

    	$('#psignature').prop('contenteditable', true);
    	$('#psignature').css({"border-style": "outset"});
    	*/
	...
	$('#saveReport').on('click', async function(e){
    	...
    	$('#remarks_report').prop('contenteditable', false);
    	$('#remarks_report').css({"border-style": "none"});

    	$('#usignature').prop('contenteditable', false);
    	$('#usignature').css({"border-style": "none"});

    	$('#psignature').prop('contenteditable', false);
    	$('#psignature').css({"border-style": "none"});
