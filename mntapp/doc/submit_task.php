...
if (isset($_POST['completedData'])){
    ...
    $qry = "INSERT INTO `mnt_report` (`id`, `name`, `json`, `html`, `pdf`, `docx`, `maintenance_date`,`before_image`,`after_image`, `checklist_id`) 
        ...
        $qry="UPDATE `mnt_report` SET `json`='$jsondata',`maintenance_date`='$maint_date' WHERE `checklist_id`='$checklist_id'"; 
    }
    //echo $jsondata;
    //$result = mysqli_query($con,$query);
    if($con->query($qry) === TRUE) {
        echo "Record created successfully for ".$qry;
    } else {
    ...
    $status = "For Approval";
    $sql = "UPDATE `tasks` SET `status`='$status',`comments`='$comment' ,`reassigned_to`='$project_manager',`end_date`='$maint_date' WHERE `id`='$task_id'";
    if($con->query($sql) === TRUE) {
        echo "Record updated successfully for ".$sql;
    } else {
