							...
							altEditor.prototype,
							{
								...
								_openEditModal : function() {
									...
									for ( var j in columnDefs) {
										var arrIndex = "['"
												+ columnDefs[j].name.toString()
														.split(".")
														.join("']['") + "']";
										var selectedValue = eval("adata.data()[0]"
												+ arrIndex);
										var jquerySelector = "#"
												+ columnDefs[j].name.toString()
														.replace(/\./g, "\\.");
										$(selector).find(jquerySelector).val(
								...
								/**
								 * Complete DataTable.context[0].aoColumns with
								 * default values
								 */
									...
									for ( var i in dt.context[0].aoColumns) {
										...
										columnDefs[i] = {
											...
											options : (obj.options ? obj.options
													: []),
											readonly : (obj.readonly ? obj.readonly
													: false),
											...
											datetimepicker : (obj.datetimepicker ? obj.datetimepicker
													: false),
											editorOnChange : (obj.editorOnChange ? obj.editorOnChange
													: null)
										}
									}
									return columnDefs;
								 ...
								 *            completeColumnDefs()
								 */
								createDialog : function(columnDefs, title,
										buttonCaption, closeCaption,
										buttonClass) {
									...
									for ( var j in columnDefs) {
										...
										if (columnDefs[j].type
											...
											// the user
											data += "<div style='margin-left: initial;margin-right: initial;' class='form-group row' id='alteditor-row-"
													+ this
															._quoteattr(columnDefs[j].name)
													+ "'>";
											...
											else if (columnDefs[j].type
													.indexOf("select") >= 0) {
												var options = "";
												var optionsArray = columnDefs[j].options;
												if (optionsArray.length > 0) {
													// array-style select or
													// select2
													for (var i = 0; i < optionsArray.length; i++) {
														options += "<option value='"
																+ this
																		._quoteattr(optionsArray[i])
																+ "'>"
																+ optionsArray[i]
																+ "</option>";
													}
												} else {
													// object-style select or
													// select2
											...
											// errorlabels, but also new HTML5
											// typees (email, color, ...)
											else {
												data += "<input type='"
														+ this
									...
									$(selector)
											.on(
													'show.bs.modal',
													function() {
														var btns = '<button type="button" data-content="remove" class="btn btn-default" data-dismiss="modal">'
																+ closeCaption
																+ '</button>'
																+ '<button type="button"  data-content="remove" class="btn btn-primary" id="'
																+ buttonClass
																+ '">'
																+ buttonCaption
																+ '</button>';
									...
									// enable select 2 items, datepicker,
									// datetimepickerm
									for ( var j in columnDefs) {
											...
											$(selector)
													.find(
															"#"
																	+ columnDefs[j].name)
													.datetimepicker(
															columnDefs[j].datetimepicker);
										}
										// custom onchange triggers
										if (columnDefs[j].editorOnChange) {
											var f = columnDefs[j].editorOnChange; // FIXME
											// what
											// if
											// more
											// than
								...
								reloadOptions : function($select, options) {
									...
									$select.val(oldValue); // if still present,
									// of course
			...
			$(document).on('preInit.dt.altEditor', function(e, settings, json) {
				...
				if (init || defaults) {
					...
					if (init !== false) {
						...
						// e.target is the underlying jQuery object, e.g.
						// $('#mytable')
